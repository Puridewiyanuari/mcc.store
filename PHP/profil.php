<?php
   ...
   $query = mysqli_query($conn, "SELECT * FROM my_admin WHERE admin_id = '".$_SESSION['id']."'");
    ...
    <div class="section">
        <div class="countainer">
            ...
            if(isset($_POST['submit'])){
                ...
                $update = mysqli_query($conn, "UPDATE my_admin SET
                            admin_name = '".$nama."',
                            username = '".$user."',
                            admin_telp = '".$hp."',
                            admin_email = '".$email."',
...
;
                ...
                if($pass2 != $pass1){
                    echo '<script>alert("Konfirmasi Password Baru Tidak Sesuai")
                    </script>' ;  
                }else{
                    $u_pass = mysqli_query($conn, "UPDATE my_admin SET
                                password ='".MD5($pass1)."'
                                WHERE admin_id = '".$d->admin_id."' ");
                    if($u_pass){
                        echo '<script>alert("Ubah Data Berhasil")</script>';
